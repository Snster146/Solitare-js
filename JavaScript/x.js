...
function addAceFromTableuToFoundatoin(card,fromIndex){
    ...
    for (let i = 0; i < foundationpiles.length; i++) {
        // document.getElementById("debug-label").innerHTML=foundationState[i];
        if (foundationState[i][0]!==null){
            continue;
        }
        else{
...
function displayNextCardInTableu(fromIndex){
    // document.getElementById("debug-label")=alltableu;
...
//     }
    ...
    if (canMoveTableaucard(card1,card2)) {
        document.getElementById("debug-label").innerHTML="can move";
        moveToTableauPile(card1,toIndex);
...
function canMoveTableaucard(card1,card2){
    ...
    cardOrder.indexOf(card2Num) - cardOrder.indexOf(card1Num) === 1 ;
}
...
function moveToTableauPile(card,toIndex){
    let targetPile = document.getElementById(`tableau-${toIndex + 1}`);
    let newImg = document.createElement("img");
...
// to add on top of other foundation cards, e.g. S2->SA
function addStockToFoundation(card){

    for (let i=0;i<foundationState.length;i++){

        if (canAddStockToFoundation(card,foundationState[i])){
            // logic to add tableau card to foundation pile
                addCardImgToFoundation(card,i);

            return true;
        }
    }
    }
function canAddStockToFoundation(card,foundationState){
    let cardSet=card[0];
    let cardNum = card.slice(1);

    let currFoundationCard=foundationState[0]
    let currFoundationSet = currFoundationCard[0];
    let currFoundationNum = currFoundationCard.slice(1);

    if (cardSet===currFoundationSet && 
        cardOrder.indexOf(cardNum)-cardOrder.indexOf(currFoundationNum)===1 && 
        currFoundationCard!==null){
...
function addCardImgToFoundation(card,foundationIndex){
    let cardimg =getCardImg(card);
    let newImg = document.createElement("img");
    newImg.src = cardimg; 
